...
func (l *InitPartLogic) InitPart(req *types.InitPartRequest, uid string) (resp *types.InitPartReply, err error) {
	...
	// 参数校验
	if req.Md5 == "" {
		resp.Code = define.FILE_MD5_EMPTY
		return
	}
	...
	defer func() {
		...
		if err != nil {
			session.Rollback()
			resp.Code = define.INIT_PART_FAILED
		} else {
			session.Commit()
		}
	}()
	...
	if !has {
		...
		userFile.Type = define.FileTypeUndone
		...
		err := FolderSizeChange(session, define.FolderSizeChangeIncrease, pid, bucket.Size, 1)
		if err != nil {
			panic(define.INIT_PART_FAILED)
		}
		resp.Data.Fid = fid

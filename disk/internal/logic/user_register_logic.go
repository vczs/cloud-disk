...
func (l *UserRegisterLogic) UserRegister(req *types.UserRegisterRequest) (resp *types.UserRegisterReply, err error) {
	...
	if req.Code == "" {
		resp.Code = config.EMAIL_CODE_EMPTY
		return
	}
	...
	if err != nil || code != req.Code {
		resp.Code = config.EMAIL_CODE_ERR
		return resp, nil
	}
	// 判断用户名是否存在
	...
	if err != nil || num > 0 {
		if num > 0 {
			resp.Code = config.USER_NAME_HAS
			return resp, nil
		}
		return nil, err
	}
	// 数据持久化
	user := &models.UserBasic{Uid: helper.GetUid(), Name: req.Name, Password: helper.Md5(req.Password), Email: req.Email, TotalVolume: int64(config.DefaultVolume)}
	num, err = l.svcCtx.Engine.Insert(user)
	if err != nil || num < 1 {
		if num < 1 {
			resp.Code = config.USER_REGISTER_ERR
			return resp, nil
		}
		return nil, err
	}
